...
void init_game_board(game_board board) {
    ...
    /* Set the central grid of size CENTRE_SIZE to an alternating RED BLUE layout */
    tiles_added = 0;
    ...
    for (i = BOARD_WIDTH / 2; i < BOARD_WIDTH / 2 + CENTRE_SIZE; i++) {
        ...
        for (j = BOARD_HEIGHT / 2; j < BOARD_HEIGHT / 2 + CENTRE_SIZE; j++) {
            /* If the current coord is an even tile added set it to red, otherwise set it to blue */
...
void display_board(game_board board, struct player *first, struct player *second) {
    ...
    /* Print the other player's name, score and token colour - if they both have the same name they still know who's turn it is*/
    if (second->token == RED) {
